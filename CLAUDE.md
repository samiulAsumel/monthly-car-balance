...
- **Treat all stored data as untrusted.** Cloud data can arrive from other devices, so run user-supplied strings through `esc()` before inserting into HTML. Load paths also self-heal known corruption (duplicate dates, numeric-keyed objects coerced back to arrays, and — via `normalizeRow()` in `src/formula.js` — a row with a missing/short/malformed `del`/`imp`/`bal`/`ob`). `normalizeRow()` is the one place this healing actually happens; don't reintroduce a `validateDB()`-style check that flags a field as invalid without calling it, or corruption gets logged but not fixed.
...
- Single page with tabs switched by `showPage(p, el)` → `daily | chart | report | settings | transfer`; each tab lazily calls its render function (`renderTable`, `renderCharts`, `renderReport`, `renderSettings`, `renderTransferPage`). `showPage()` syncs the active tab across **both** `.ntab` (desktop top nav) and `.bnav-tab` (fixed bottom nav, shown instead below 700px) by matching a shared `data-page` attribute — not by touching the `el` argument, which exists only for backward compatibility with older call sites. Add a new tab to *both* markup blocks in `index.html`, each with the same `data-page` value, or it won't show up in the one that's currently hidden.
- Mobile (≤700px) chrome heights are measured, not hardcoded: `updateStickyHeaderOffsets()` (`app.js`) feeds `.topbar`/`.nav`/`.bnav`'s real rendered heights into `--topbar-h`/`--nav-h`/`--bnav-h`, which `.nav`, `.mbar`, `#toast-stack`, `.cmdk-fab`, and `.page.on`'s bottom padding all read from — so a taller topbar (the brand block wraps at narrow widths) or a hidden `.nav` never desyncs the sticky offsets. When adding a CSS rule that's `display:none` by default and shown via a `max-width` media query, the unconditional rule **must** come before that media query in source order — a rule placed after wins the cascade regardless of whether the media query matches (bit this once already with `.bnav` and `.cmdk-fab`).
...
- `setDirty(v, schedule = true)` after any data mutation controls the "Saved"/"Unsaved" badge; `schedule` (default `true`) also arms the 3s autosave timer — pass `false` from code that's only correcting the badge to reflect a save already in flight or already failed, so it doesn't re-arm the very timer that got it there. `saveLS()` only persists to `localStorage` — it does **not** touch the dirty flag; every caller sets the badge itself once it knows whether the *cloud* write succeeded, so "Saved" is never shown for a change that only made it to `localStorage`. A module-level `saveInFlight` flag (set in `saveToFirebase()`) makes `performAutoSave()` skip a save while a manual save is already in flight, rather than racing it against the same `cloudBaseSha`. `requireLogin(cb)` gates edits; `showError` / `showSuccess` / overlays handle user feedback.
- Keyboard shortcuts are defined in `SHORTCUTS` (Ctrl+S save, Ctrl+Z undo, 1–5 tabs, F1 help, etc.); Ctrl+K opens the command palette (`src/command-palette.js`) — checked before `SHORTCUTS`' own INPUT/TEXTAREA bail-out, so it opens even from a focused table cell. The palette traps Tab (its input is the only focusable node while it's open), exposes listbox/option ARIA roles with `aria-activedescendant`, and restores focus to whatever triggered it on close. On mobile, `.cmdk-fab` (a floating button, ≤700px only) is the touch entry point — the palette itself becomes a full-viewport sheet at that width, same underlying logic either way.
- Mobile-only interactions: `initDayCardSwipe()` (bound once, on `#day-cards` — the container persists across `renderDayCards()` rebuilds) turns a horizontal touch drag into `navigateMonth(-1/1)`, ignoring touches that start on an `<input>`. `showInstallBanner()` wires the real `beforeinstallprompt` flow (a dismissed choice is remembered in `localStorage` under `installBannerDismissed`); it's prepended ahead of `.topbar` in normal document flow specifically so it needs no sticky-offset changes — don't fixed-position it without re-deriving `--topbar-h`.
- Tap/click feedback (scale + ripple) is delegated on `document` from `initializeVisualFeedback()`, matched against the `TAP_FEEDBACK_SELECTOR` constant — add a new interactive element type there rather than wiring `addEventListener` on individual elements, since day-cards/month-bar/report headers are all rebuilt from scratch on re-render and a per-element listener would silently stop covering them.
